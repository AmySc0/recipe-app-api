...
class PrivateRecipesApiTests(TestCase):
    ...
    def test_retrieve_recipes(self):
        ...
        sample_recipe(user=self.user, title='Creamy Mushroom Sauce')
    ...
    def test_recipes_limited_to_user(self):
        """Test that only recipes for the auth'd user are returned"""
        user2 = get_user_model().objects.create_user(
            'other@londonappdev.com',
            'testpass'
        )
        sample_recipe(user=user2)
        sample_recipe(user=self.user, title='Beef Stroganoff')
    ...
    def test_partial_recipe_update(self):
        """Test updating part of a recipe details (patch)"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
    ...
    def test_full_recipe_update(self):
        ...
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        payload = {'title': 'Spaghetti Carbonara', 'time_mins': 25, 'price': 5}
        url = detail_url(recipe.id)
        self.client.put(url, payload)

        recipe.refresh_from_db()
        self.assertEqual(recipe.title, payload['title'])
        self.assertEqual(recipe.time_mins, payload['time_mins'])
